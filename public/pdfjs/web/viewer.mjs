...
class CommentManager {
  ...
  static _makeCommentColor(color, opacity) {
    return this.#hasForcedColors
      ? null
      : findContrastColor(applyOpacity(...color, opacity ?? 1), CSSConstants.commentForegroundColor);
  }
  destroy() {
    this.#dialog.destroy();
    this.#sidebar.hide();
...
class CommentSidebar {
  ...
  #setText(element, richText, contentsObj) {
    ...
    const html =
      richText?.str && (!contentsObj?.str || richText.str === contentsObj.str) ? richText.html : contentsObj?.str;
    renderRichText(
      {
        html,
...
class PDFPageDetailView extends BasePDFPageView {
  ...
  async draw() {
    ...
    const hideUntilComplete =
      this.pageView.renderingState === RenderingStates.FINISHED || this.renderingState === RenderingStates.FINISHED;
    if (this.renderingState !== RenderingStates.INITIAL) {
      console.error('Must be in new state before drawing');
      this.reset();
    }
...
class PDFViewer {
  ...
  #initializePermissions(permissions) {
    ...
    this.#printingAllowed =
      permissions.includes(PermissionFlag.PRINT_HIGH_QUALITY) || permissions.includes(PermissionFlag.PRINT);
    this.eventBus.dispatch('printingallowed', {
      source: this,
      isAllowed: this.#printingAllowed,
    });
  ...
  forceRendering(currentlyVisiblePages) {
    ...
    const ignoreDetailViews =
      this.#scaleTimeoutId !== null ||
      (this.#scrollTimeoutId !== null && visiblePages.views.some((page) => page.detailView?.renderingCancelled));
      ...
      this.#drawSVG.addEventListener(
        ...
        (evt) => {
          ...
          if (drawPoints.length >= 4) {
            ...
            this.#drawPathString += `C${(x1 + 5 * x2) / 6} ${(y1 + 5 * y2) / 6} ${(5 * x2 + x) / 6} ${
              (5 * y2 + y) / 6
            } ${(x2 + x) / 2} ${(y2 + y) / 2}`;
          } else {
